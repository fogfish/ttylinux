...
PKG_URL="http://www.erlang.org/download/"
PKG_TAR="otp_src_R15B02.tar.gz"
PKG_SUM=""
...
./configure \
        --build=${MACHTYPE} \
	--enable-bootstrap-only || return 1
...
cd "${PKG_NAME}-${PKG_VERSION}"
source "${TTYLINUX_XTOOL_DIR}/_xbt_env_set"
mkdir --force ${TTYLINUX_SYSROOT_DIR}/usr/otp
PATH="${XBT_BIN_PATH}:${PATH}" make \
